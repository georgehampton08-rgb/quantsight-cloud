...
const NAV_ITEMS = [
    ...
    { name: 'Matchup Engine', Icon: Swords,        path: '/matchup',     comingSoon: true },
    { name: 'The Pulse',      Icon: Heart,         path: '/pulse' },
    { name: 'Matchup Lab',    Icon: Microscope,    path: '/matchup-lab', comingSoon: true },
    { name: 'Team Central',   Icon: Shield,        path: '/team',        comingSoon: true },
            ...
            <div className="flex flex-1 flex-col gap-2 overflow-y-auto scrollbar-premium pr-2">
                {NAV_ITEMS.map((item, index) => (
                    <React.Fragment key={item.name}>
                        ...
                        {item.comingSoon ? (
                            <div
                                className="relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium border-l-2 border-transparent opacity-40 cursor-not-allowed select-none overflow-hidden"
                            >
                                <item.Icon className="w-4 h-4 flex-shrink-0 text-pro-muted" />
                                <span className="whitespace-nowrap text-pro-muted">{item.name}</span>
                                {/* Overlay */}
                                <span className="absolute inset-0 flex items-center justify-center rounded-xl bg-pro-bg/60 backdrop-blur-[1px]">
                                    <span className="text-[9px] font-bold tracking-widest uppercase text-amber-400/90">
                                        Coming Soon
                                    </span>
                                </span>
                            </div>
